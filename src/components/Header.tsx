...
export const Header = () => {
  ...
  const navItems = ['О группе', 'Музыка', 'Галерея', 'Видео', 'Магазин', 'Контакты'];
    ...
    <header className={`fixed w-full z-50 transition-all diration-300 ${isScrolled ? 'bg-black/90 py-2' : 'bg-black/50 py-4'}`}>
      <div className='container mx-auto px-4 flex items-center justify-between'>
          ...
          <nav className='hidden md:flex space-x-6'>
            {navItems.map((item) => (
              <Link 
                ...
                className='text-white hover:yellow-400 transition-colors font-medium'
              >
                {item}
              </Link>
            ))}
        ...
        <div className="flex items-center space-x-4">
          <a href="https://vk.com/bogobes" target="_blank" rel="noopener noreferrer" aria-label="VKontakte">
            <svg className="w-6 h-6 text-white hover:text-blue-500 transition-colors" fill="currentColor" viewBox="0 0 24 24">
          ...
          <a href="https://youtube.com/bogobes" target="_blank" rel="noopener noreferrer" aria-label="YouTube">
            <svg className="w-6 h-6 text-white hover:text-red-600 transition-colors" fill="currentColor" viewBox="0 0 24 24">
              <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
            </svg>
          </a>
          <a href="https://music.yandex.ru/bogobes" target="_blank" rel="noopener noreferrer" aria-label="Яндекс Музыка">
            <svg className="w-6 h-6 text-white hover:text-orange-500 transition-colors" fill="currentColor" viewBox="0 0 24 24">
